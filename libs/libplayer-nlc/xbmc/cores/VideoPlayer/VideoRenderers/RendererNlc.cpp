...
#include "VideoShaders/YUV2RGBShaderNlc.h"
#include "VideoShaders/VideoFilterShaderNlc.h"
...
//============================================================================
CRendererNlc::YUVBUFFER::YUVBUFFER()
{
...
//============================================================================
CRendererNlc::CRendererNlc()
: m_INlc( INlcRender::getINlcRender() )
{
    LogModule( eLogVideoRender, LOG_INFO, "Constructing CRendererNlc" );
...
//============================================================================
CBaseRenderer* CRendererNlc::Create( CVideoBuffer *buffer )
{
    return new CRendererNlc();
}

//============================================================================
bool CRendererNlc::Register()
{
    VIDEOPLAYER::CRendererFactory::RegisterRenderer( "default", CRendererNlc::Create );
    return true;
}

//============================================================================
bool CRendererNlc::ValidateRenderTarget()
{
    if( !m_bValidated )
    {
...
//============================================================================
bool CRendererNlc::Configure( const VideoPicture &picture, float fps, unsigned int orientation )
{
...
//============================================================================
bool CRendererNlc::ConfigChanged( const VideoPicture &picture )
{
...
//============================================================================
int CRendererNlc::NextYV12Texture()
{
    return ( m_iYV12RenderBuffer + 1 ) % m_NumYV12Buffers;
}

//============================================================================
void CRendererNlc::AddVideoPicture( const VideoPicture &picture, int index )
{
    //LogModule( eLogVideoIo, LOG_DEBUG, "CRendererNlc::AddVideoPicture YUVBUFFER %d", index );
...
//============================================================================
void CRendererNlc::ReleaseBuffer( int idx )
{
...
//============================================================================
void CRendererNlc::CalculateTextureSourceRects( int source, int num_planes )
{
...
//============================================================================
void CRendererNlc::LoadPlane( YUVPLANE& plane, int type,
...
//============================================================================
bool CRendererNlc::Flush(bool saveBuffers)
{
    if( !m_bValidated )
        return false;
...
//============================================================================
void CRendererNlc::RenderUpdate( int index, int index2, bool clear, unsigned int flags, unsigned int alpha )
{
    ...
    {
        //LogModule( eLogVideoIo, LOG_DEBUG, "CRendererNlc::RenderUpdate YUVBUFFER %d", index );
        Render( flags, index );
    }
...
//============================================================================
void CRendererNlc::RenderUpdateVideo( bool clear, unsigned int flags, unsigned int alpha )
{
    if( !m_bConfigured )
        return;
...
//============================================================================
void CRendererNlc::UpdateVideoFilter()
{
    if( !m_bValidated )
    {
        return;
    ...
    {
        CLog::Log( LOGWARNING, "CRendererNlc::UpdateVideoFilter - chosen scaling method %d, is not supported by renderer", ( int )m_scalingMethod );
        m_scalingMethod = VS_SCALINGMETHOD_LINEAR;
    }
    ...
    case VS_SCALINGMETHOD_CUBIC_0_1:
        ...
        m_pVideoFilterShader = new Shaders::ConvolutionFilterShaderNlc( m_INlc, m_scalingMethod );
...
void CRendererNlc::LoadShaders( int field )
{
    m_reloadShaders = 0;
    ...
    case RENDER_METHOD_GLSL:
        ...
        //if( glCreateProgram() )
        {
            ...
            m_pYUVProgShader = new Shaders::YUV2RGBProgressiveShaderNlc( m_INlc, m_iFlags, shaderFormat );
            m_INlc.verifyGlState();
            m_pYUVProgShader->SetConvertFullColorRange( m_fullRange );
            m_INlc.verifyGlState();
#ifdef ENABLE_GLES_SHADERS
            m_pYUVBobShader = new Shaders::YUV2RGBBobShaderNlc( m_INlc, m_iFlags, shaderFormat );
            m_INlc.verifyGlState();
    ...
    {
        CLog::Log( LOGDEBUG, "CRendererNlc: Reorder drawpoints due to method change from %i to %i", m_oldRenderMethod, m_renderMethod );
        ReorderDrawPoints();
        m_oldRenderMethod = m_renderMethod;
    }
}
...
//============================================================================
void CRendererNlc::ReleaseShaders()
{
    if( m_pYUVProgShader )
    {
...
//============================================================================
void CRendererNlc::UnInit()
{
    if( !VxIsAppShuttingDown() )
    {
        LogModule( eLogVideoRender, LOG_DEBUG, "CRendererNlc::UnInit: Cleaning up GL resources" );
...
//============================================================================
inline void CRendererNlc::ReorderDrawPoints()
{
    CBaseRenderer::ReorderDrawPoints();//call base impl. for rotating the points
}

//============================================================================
bool CRendererNlc::CreateTexture( int index )
{
...
//============================================================================
void CRendererNlc::DeleteTexture( int index )
{
    ReleaseBuffer( index );
...
//============================================================================
bool CRendererNlc::UploadTexture( int index )
{
    ...
    //LogModule( eLogVideoIo, LOG_DEBUG, "CRendererNlc::UploadTexture %d", index );
...
//============================================================================
void CRendererNlc::Render( unsigned int flags, int index )
{
    ...
    //LogModule( eLogVideoIo, LOG_DEBUG, "CRendererNlc::Render %d", index );
...
//============================================================================
void CRendererNlc::RenderSinglePass( int index, int field )
{
    YUVPLANE( &planes )[ YuvImage::MAX_PLANES ] = m_buffers[ index ].fields[ field ];

    //LogModule( eLogVideoIo, LOG_DEBUG, "CRendererNlc::RenderSinglePass %d", index );
...
//============================================================================
void CRendererNlc::RenderToFBO( int index, int field, bool weave /*= false*/ )
{
    YUVPLANE( &planes )[ YuvImage::MAX_PLANES ] = m_buffers[ index ].fields[ field ];
    ...
    LogModule( eLogVideoIo, LOG_DEBUG, "CRendererNlc::RenderToFBO %d", index );
...
//============================================================================
void CRendererNlc::RenderFromFBO()
{
    //glActiveTexture( GL_TEXTURE0 );
    m_INlc.setActiveGlTexture( 0 );
...
//============================================================================
bool CRendererNlc::RenderCapture( CRenderCapture* capture )
{
    if( !m_bValidated )
        return false;
...
//********************************************************************************************************
bool CRendererNlc::UploadYV12Texture( int source )
{
    YUVBUFFER& buf = m_buffers[ source ];
    YuvImage* im = &buf.image;
...
//============================================================================
void CRendererNlc::DeleteYV12Texture( int index )
{
...
bool CRendererNlc::CreateYV12Texture( int index )
{
...
//********************************************************************************************************
bool CRendererNlc::UploadNV12Texture( int source )
{
    YUVBUFFER& buf = m_buffers[ source ];
    YuvImage* im = &buf.image;
...
//============================================================================
bool CRendererNlc::CreateNV12Texture( int index )
{
    // since we also want the field textures, pitch must be texture aligned
    YUVBUFFER& buf = m_buffers[ index ];
    YuvImage &im = buf.image;
...
//============================================================================
void CRendererNlc::DeleteNV12Texture( int index )
{
    YUVBUFFER& buf = m_buffers[ index ];
    YuvImage &im = buf.image;
...
//********************************************************************************************************
void CRendererNlc::SetTextureFilter( GLenum method )
{
    for( int i = 0; i < m_NumYV12Buffers; i++ )
    {
...
//============================================================================
bool CRendererNlc::Supports( ERENDERFEATURE feature ) const
{
...
//============================================================================
bool CRendererNlc::SupportsMultiPassRendering()
{
    return true;
}

//============================================================================
bool CRendererNlc::Supports( ESCALINGMETHOD method ) const
{
...
//============================================================================
CRenderInfo CRendererNlc::GetRenderInfo()
{
    CRenderInfo info;
...
//============================================================================
bool CRendererNlc::IsGuiLayer()
{
    return true; // this is wrong.. but works only render video layer
}

//============================================================================
AVColorPrimaries CRendererNlc::GetSrcPrimaries( AVColorPrimaries srcPrimaries, unsigned int width, unsigned int height )
{
