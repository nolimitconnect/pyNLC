...
CGUIFontTTFNlc::CGUIFontTTFNlc( const std::string& strFileName )
...
CGUIFontTTFNlc::~CGUIFontTTFNlc( void )
{
    // It's important that all the CGUIFontCacheEntry objects are
    // destructed before the CGUIFontTTFNlc goes out of scope, because
    // our virtual methods won't be accessible after this point
    //m_dynamicCache.Flush();
    DeleteHardwareTexture();
}

bool CGUIFontTTFNlc::FirstBegin()
{
...
CVertexBuffer CGUIFontTTFNlc::CreateVertexBuffer( const std::vector<SVertex> &vertices ) const
{
    return INlcRender::getINlcRender().createVertexBuffer( const_cast<CGUIFontTTFNlc*>(this), vertices );
...
void CGUIFontTTFNlc::DestroyVertexBuffer( CVertexBuffer &buffer ) const
{
    return INlcRender::getINlcRender().destroyVertexBuffer( const_cast<CGUIFontTTFNlc*>(this), buffer );
...
std::unique_ptr<CTextureBase> CGUIFontTTFNlc::ReallocTexture( unsigned int& newHeight )
{
    ...
    {
        CLog::Log( LOGERROR, "GUIFontTTFNlc::CacheCharacter: Error creating new cache texture for size %f", m_height );
        return nullptr;
    }
...
bool CGUIFontTTFNlc::CopyCharToTexture( FT_BitmapGlyph bitGlyph, unsigned int x1, unsigned int y1, unsigned int x2, unsigned int y2 )
{
...
void CGUIFontTTFNlc::DeleteHardwareTexture()
{
    INlcRender::getINlcRender().deleteHardwareTexture( const_cast< CGUIFontTTFNlc* >( this ) );
}

void CGUIFontTTFNlc::CreateStaticVertexBuffers( void )
{
    if( m_staticVertexBufferCreated )
        return;

    CGUIFontTTFNlc fontQt( "" );
...
//}
//
void CGUIFontTTFNlc::DestroyStaticVertexBuffers( void )
{
    if( !m_staticVertexBufferCreated )
        return;
...
unsigned int CGUIFontTTFNlc::m_elementArrayHandle;
bool CGUIFontTTFNlc::m_staticVertexBufferCreated;
